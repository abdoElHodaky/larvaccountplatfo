...
export const CustomReportBuilder: React.FC<CustomReportBuilderProps> = ({
    ...
    const [activeStep, setActiveStep] = useState<
        'basic' | 'fields' | 'filters' | 'sorting' | 'preview'
    >('basic');
    ...
    const handleUpdateSort = (index: number, updates: Partial<ReportSort>) => {
        setReport((prev) => ({
            ...prev,
            sorting: prev.sorting.map((sort, i) => (i === index ? { ...sort, ...updates } : sort)),
        }));
    };
                        ...
                        <div className='space-y-6'>
                            ...
                            {report.filters.length === 0 ? (
                                <div className='text-center py-8 text-gray-500'>
                                    No filters added. Click "Add Filter" to create your first
                                    filter.
                                </div>
                            ) : (
                        ...
                        <div className='space-y-6'>
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        setReport((prev) => ({ ...prev, groupBy: e.target.value }))
                                    }
                                    className='w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500'
                                >
                                ...
                                <div className='grid grid-cols-1 md:grid-cols-2 gap-4 text-sm'>
                                    <div>
                                        ...
                                        <ul className='mt-1 space-y-1'>
                                            {report.fields.map((fieldId) => (
                                                <li key={fieldId} className='text-gray-600'>
                                                    • {getFieldName(fieldId)}
                                                </li>
                                            ))}
                                        </ul>

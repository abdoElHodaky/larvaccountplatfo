...
import { Box, VStack, HStack, Text, IconButton, useColorModeValue } from '@chakra-ui/react';
...
export const ReportCanvas: React.FC<ReportCanvasProps> = memo(
    ({
        ...
        const handleWidgetSelect = useMemoizedCallback(
            (widget: ReportWidget) => {
                if (onWidgetSelect) {
        ...
        const handleWidgetDelete = useMemoizedCallback(
            (widgetId: string) => {
            ...
            drop: (item: any, monitor) => {
                if (!monitor.didDrop()) {
                    ...
                    if (clientOffset && canvasRect) {
                        const x = Math.floor(
                            (clientOffset.x - canvasRect.left) / (canvasRect.width / cols.lg)
                        );
                        const y = Math.floor(
                            (clientOffset.y - canvasRect.top) / (rowHeight + margin[1])
                        );
        ...
        const renderWidget = useMemoizedCallback(
            (widget: ReportWidget) => {
                ...
                return (
                    ...
                    >
                        ...
                        {/* Widget Content */}
                        <Box p={2} h={isEditing ? 'calc(100% - 32px)' : 'full'} overflow='hidden'>

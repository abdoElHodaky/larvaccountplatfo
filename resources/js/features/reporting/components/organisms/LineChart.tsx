...
export const LineChart: React.FC<LineChartProps> = memo(
    ({
        ...
        const xAxisFormatter = useMemoizedCallback(
            (value: any) => {
                if (formatXAxis) {
        ...
        const yAxisFormatter = useMemoizedCallback(
            (value: any) => {
                if (formatYAxis) {
        ...
        const tooltipFormatter = useMemoizedCallback(
        ...
        const chartComponent = useMemo(
            () => (
                <ResponsiveContainer width='100%' height='100%'>
                    <RechartsLineChart data={processedData} margin={margin} ref={chartRef}>
                        {showGrid && (
        ...
        return (
            <ChartContainer {...containerProps} onExport={handleExport}>
                {chartComponent}
            </ChartContainer>
        );
    }
...
export const FinancialLineChart: React.FC<LineChartProps> = memo((props) => (
    <LineChart
        ...
        variant='financial'
        formatYAxis={(value) =>
            FinancialPerformanceUtils.formatCurrency(Number(value) || 0, 'USD', 0)
        }

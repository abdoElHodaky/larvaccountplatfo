...
export const dashboardModel = createModel<RootModel>()({
    ...
    reducers: {
        ...
        updateLayout: (state, updatedLayout: DashboardLayout) => ({
            ...
            currentLayout:
                state.currentLayout?.id === updatedLayout.id ? updatedLayout : state.currentLayout,
        }),
        ...
        setWidgetLoading: (state, payload: { widgetId: string; loading: boolean }) => ({
            ...state,
            widgets: state.widgets.map((widget) =>
                widget.id === payload.widgetId ? { ...widget, loading: payload.loading } : widget
            ),
        }),
    ...
    effects: (dispatch) => ({
        ...
        async fetchLayouts() {
            ...
            try {
                ...
                if (!this.currentLayout && response.data.length > 0) {
                    const defaultLayout =
                        response.data.find((l) => l.isDefault) || response.data[0];
        ...
        // Auto-refresh
        async startAutoRefresh(intervalMs: number = 300000) {
            // 5 minutes default

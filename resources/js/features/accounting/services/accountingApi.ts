                ...
                update: (cache, { data: mutationData }) => {
                    ...
                    if (existingAccounts) {
                        cache.writeQuery({
                            ...
                            data: {
                                accounts: [
                                    ...(existingAccounts as any).accounts,
                                    mutationData.createAccount,
                                ],
                            },
                        });
                    }
                ...
                update: (cache, { data: mutationData }) => {
                    ...
                    if (existingEntries) {
                        cache.writeQuery({
                            ...
                            data: {
                                journalEntries: [
                                    ...(existingEntries as any).journalEntries,
                                    mutationData.createJournalEntry,
                                ],
                            },
                        });
                    }
    ...
    async reconcileTransactions(transactionIds: string[]): Promise<ApiResponse<boolean>> {
      ...
      `;
            ...
            if (response.reconcileTransactions.errors?.length > 0) {
                console.warn(
                    'Some transactions failed to reconcile:',
                    response.reconcileTransactions.errors
                );
            }

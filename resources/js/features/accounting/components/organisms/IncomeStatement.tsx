...
export const IncomeStatement: React.FC<IncomeStatementProps> = memo(({
  ...
  // Handle item click
  const handleItemClick = useMemoizedCallback((item: IncomeStatementItem) => {
    if (onItemClick) {
      onItemClick(item);
    }
  }, [onItemClick]);
  ...
  const renderTrendIndicator = useMemoizedCallback((item: IncomeStatementItem) => {
    ...
    <VStack spacing={4} align="stretch">
      <HStack justify="space-between" align="center">
        ...
        <VStack align="flex-end" spacing={1}>
          <Text fontSize="lg" fontWeight="bold" color={data.totals.netIncome >= 0 ? positiveColor : negativeColor}>
  ...
  const keyMetrics = useMemo(() => (
    <HStack spacing={8} justify="center" wrap="wrap">
      <VStack spacing={1}>
        ...
        <Text fontSize="lg" fontWeight="bold" color={data.totals.grossProfitMargin >= 0 ? positiveColor : negativeColor}>
      ...
      <VStack spacing={1}>
        ...
        <Text fontSize="lg" fontWeight="bold" color={data.totals.operatingMargin >= 0 ? positiveColor : negativeColor}>
      ...
      <VStack spacing={1}>
        ...
        <Text fontSize="lg" fontWeight="bold" color={data.totals.netProfitMargin >= 0 ? positiveColor : negativeColor}>
  ...
  const tableContent = useMemo(() => {
    ...
    return (
      <Fragment>
        ...
        {renderSectionHeader('COST OF GOODS SOLD', -data.totals.totalCostOfGoodsSold)}
        {groupedItems.cost_of_goods_sold.map((item, index) => renderItemRow(item, index))}
        ...
        {renderSectionHeader('OPERATING EXPENSES', -data.totals.totalOperatingExpenses)}
        {groupedItems.operating_expense.map((item, index) => renderItemRow(item, index))}
        ...
        {groupedItems.other_income.length > 0 && (
          <Fragment>
            {renderSectionHeader('OTHER INCOME', data.totals.totalOtherIncome)}
            {groupedItems.other_income.map((item, index) => renderItemRow(item, index))}
          </Fragment>
        )}
        ...
        {groupedItems.other_expense.length > 0 && (
          <Fragment>
            {renderSectionHeader('OTHER EXPENSES', -data.totals.totalOtherExpenses)}
            {groupedItems.other_expense.map((item, index) => renderItemRow(item, index))}
          </Fragment>
        )}

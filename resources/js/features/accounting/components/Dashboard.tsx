...
import {
    ...
    StaggerAnimation,
} from '../../../shared/components/animation/FragmentAnimationWrapper';
...
export const Dashboard: React.FC<DashboardProps> = ({
    ...
    } = useAccountBalances(orgId, state.selectedDateRange.end, { enabled: !!orgId });
    ...
    } = useTrialBalance(orgId, state.selectedDateRange.end, {
    ...
    const handleFilterChange = useCallback(
        (filterOptions: Partial<AccountingState['filterOptions']>) => {
            setState((prev) => ({
                ...prev,
    ...
    const handleAccountUpdate = useCallback(
        async (accountId: string, updates: any) => {
            ...
            <div className='accounting-dashboard'>
                ...
                <div className='accounting-content'>
                    {state.viewMode === 'overview' && (
                        <Fragment>
                            ...
                            >
                                <StaggerAnimation staggerDelay={0.15} className='overview-grid'>
                                    {[
                    ...
                    {state.viewMode === 'accounts' && (
                        <Fragment>
                            <FragmentAnimationWrapper variant='slideLeft' className='accounts-view'>
                                <Accounts
                            ...
                            >
                                <TransactionList
                                    ...
                                    onTransactionUpdate={(id, updates) =>
                                        console.log('Transaction update:', id, updates)
                                    }
                                    filterOptions={state.filterOptions}
                                    enableRealtime={enableRealtime}
                                    enableCollaboration={enableCollaboration}
                    ...
                    {state.viewMode === 'reports' && (
                        <Fragment>
                            <FragmentAnimationWrapper variant='scaleIn' className='reports-view'>
                                <StaggerAnimation staggerDelay={0.2} className='reports-grid'>
                                    {[
                ...
                {enableCollaboration && selectedAccountId && accountCollaborators.length > 0 && (
                    <div className='collaboration-status'>
                        ...
                        {accountLocked && (
                            <div className='locked-indicator'>🔒 Account locked for editing</div>
                        )}
                    </div>
                )}
            </div>

...
import {
    useGraphQLQuery,
    useGraphQLMutation,
    usePaginatedGraphQLQuery,
} from '../../../shared/hooks/useGraphQL';
...
export function useChartOfAccounts(accountType?: string) {
    ...
    const { data, loading, error, refetch } = useGraphQLQuery(GET_CHART_OF_ACCOUNTS, {
        variables: {
...
export function useTransactions(filters: TransactionFilters = {}, pageSize = 20) {
    ...
    const result = usePaginatedGraphQLQuery(GET_TRANSACTIONS, {
        variables: {
            tenantId: currentTenant?.id,
            ...filters,
        },
...
export function useUpdateTransaction() {
    ...
    const [updateTransaction, { loading, error }] = useGraphQLMutation(UPDATE_TRANSACTION, {
        refetchQueries: ['GetTransactions', 'GetDashboardData'],
    });
...
export function useDeleteTransaction() {
    ...
    const [deleteTransaction, { loading, error }] = useGraphQLMutation(DELETE_TRANSACTION, {
        refetchQueries: ['GetTransactions', 'GetDashboardData'],
    });
...
export function useTrialBalance(period: ReportPeriod, includeZeroBalances = false) {
    ...
    const { data, loading, error, refetch } = useGraphQLQuery(GET_TRIAL_BALANCE, {
        variables: {
            tenantId: currentTenant?.id,
...
export function useIncomeStatement(period: ReportPeriod, compareWithPrevious = false) {
    ...
    const { data, loading, error, refetch } = useGraphQLQuery(GET_INCOME_STATEMENT, {
        variables: {
            tenantId: currentTenant?.id,
            periodStart: period.periodStart,
            periodEnd: period.periodEnd,
...
export function useBalanceSheet(asOfDate: string, compareWithPrevious = false) {
    ...
    const { data, loading, error, refetch } = useGraphQLQuery(GET_BALANCE_SHEET, {
        variables: {
            tenantId: currentTenant?.id,
            asOfDate,
            compareWithPrevious,
...
export function useDashboardData(
    ...
    const { data, loading, error, refetch } = useGraphQLQuery(GET_DASHBOARD_DATA, {
        variables: {
            tenantId: currentTenant?.id,
            period,
        },
...
export function useAccountManagement() {
    ...
    const [createAccount, { loading: createLoading }] = useGraphQLMutation(CREATE_ACCOUNT, {
        refetchQueries: ['GetChartOfAccounts'],
    });

    const [updateAccount, { loading: updateLoading }] = useGraphQLMutation(UPDATE_ACCOUNT, {
        refetchQueries: ['GetChartOfAccounts'],
    });

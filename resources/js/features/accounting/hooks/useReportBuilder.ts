...
export function useReportBuilder(options: UseReportBuilderOptions = {}): UseReportBuilderReturn {
    const {
        autoSave = false,
    ...
    // Add to history
    const addToHistory = useMemoizedCallback(
        (newReport: ReportTemplate) => {
            setHistory((prev) => {
    ...
    const updateReport = useMemoizedCallback(
        (updater: (prev: ReportTemplate | null) => ReportTemplate | null) => {
            setReport((prev) => {
    ...
    // Load report
    const loadReport = useMemoizedCallback(
    ...
    // Export report
    const exportReport = useMemoizedCallback(
        async (format: 'json' | 'pdf' | 'png'): Promise<void> => {
            if (!report) {
    ...
    // Add widget
    const addWidget = useMemoizedCallback(
        (widget: Omit<ReportWidget, 'id'>) => {
    ...
    // Update widget
    const updateWidget = useMemoizedCallback(
    ...
    // Remove widget
    const removeWidget = useMemoizedCallback(
        (widgetId: string) => {
            updateReport((prev) => {
                if (!prev) return prev;
                return {
    ...
    const updateWidgets = useMemoizedCallback(
        (newWidgets: ReportWidget[]) => {
            updateReport((prev) => {
                if (!prev) return prev;
                return {
                    ...prev,

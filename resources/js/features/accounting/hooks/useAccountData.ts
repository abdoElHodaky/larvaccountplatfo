...
export const useAccount = (accountId: string) => {
    return useRequest(restClient.Get(`/accounts/${accountId}`), {
        initialData: null,
        cacheFor: 300000, // 5 minutes
        transform: (response) => response.data,
    });
...
export const useCreateAccount = () => {
    return useRequest((data: CreateAccountData) => restClient.Post('/accounts', data), {
        immediate: false,
        transform: (response) => response.data,
    });
};
...
export const useDeleteAccount = () => {
    return useRequest((accountId: string) => restClient.Delete(`/accounts/${accountId}`), {
        immediate: false,
    });
};

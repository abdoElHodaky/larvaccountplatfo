...
import { Account, AccountBalance, Transaction, PageProps, PaginatedData } from '@/shared/ICONSIZES';
import { 
    ArrowLeftIcon, 
    PencilIcon, 
    // ChartBarIcon,
                            ...
                            <div className="bg-white shadow rounded-lg">
                                ...
                                <div className="px-6 py-4 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <dd className="mt-1 text-sm text-gray-900">{account.subtype}</dd>
                                    </div>
                                    <div>
                                        <dt className="text-sm font-medium text-gray-500">Normal Balance</dt>
                                        <dd className="mt-1 text-sm text-gray-900 capitalize">{account.normal_balance}</dd>
                                    </div>
                                    <div>
                                    ...
                                    {account.parent && (
                                        <div>
                                            ...
                                            <dd className="mt-1">
                                                <Link
                                                    href={route('accounting.accounts.show', account.parent.id)}
                                    ...
                                    <div>
                                        <dt className="text-sm font-medium text-gray-500">Manual Entries</dt>
                                        <dd className="mt-1">
                                            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                                                ...
                                                {account.allow_manual_entries ? 'Allowed' : 'Not Allowed'}
                                            </span>
                                        </dd>
                                    </div>
                                </div>
                                    ...
                                    <div className="px-6 py-4">
                                        <ul className="space-y-3">
                                            {account.children.map((child) => (
                                                <li key={child.id}>
                                                    <Link
                                                        href={route('accounting.accounts.show', child.id)}
                                                    ...
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            <div className="text-sm text-gray-900">
                                                                {formatBalance(child.current_balance, child.currency)}
                                                            </div>
                                                        </div>
                                                    </Link>
                                                </li>
                                                ...
                                                {balanceHistory.slice(0, 10).map((balance) => (
                                                    <tr key={balance.id} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                                                            {formatBalance(balance.opening_balance, balance.currency)}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                                                            {formatBalance(balance.debit_total, balance.currency)}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                                                            {formatBalance(balance.credit_total, balance.currency)}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right font-medium">
                                                            {formatBalance(balance.closing_balance, balance.currency)}
                                                        </td>
                            ...
                            <div className="bg-white shadow rounded-lg">
                                <div className="px-6 py-4 border-b border-gray-200">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <Link
                                            href={route('accounting.transactions.index', { account_id: account.id })}
                                            className="text-sm text-indigo-600 hover:text-indigo-900"
                                        >
                                            ...
                                            {transactions.data.length > 0 ? (
                                                transactions.data.map((transaction) => (
                                                    <tr key={transaction.id} className="hover:bg-gray-50">
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                                                            {formatBalance(transaction.total_amount, transaction.currency)}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">

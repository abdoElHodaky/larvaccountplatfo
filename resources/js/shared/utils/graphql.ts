...
export function useGraphQL(options?: Partial<GraphQLClientOptions>) {
  ...
  const query = React.useCallback(async <T = any>(
    query: string,
    variables?: Record<string, any>,
  ...
  const mutate = React.useCallback(async <T = any>(
    mutation: string,
    variables?: Record<string, any>,
...
export function useFinancialGraphQL(tenantId?: string) {
  ...
  const getTrialBalance = React.useCallback(async (period: { start: string; end: string }) => {
    return query(`
  ...
  const getIncomeStatement = React.useCallback(async (period: { start: string; end: string }) => {
    return query(`
  ...
  const getBalanceSheet = React.useCallback(async (asOfDate: string) => {
    return query(`

...
class PerformanceMonitor {
    ...
    private analyzeLazyLoadMetrics() {
        ...
        // Group by component
        const byComponent = this.lazyLoadMetrics.reduce(
            (acc, metric) => {
    ...
    private analyzeRouteMetrics() {
        ...
        const preloadedAverage =
            preloadedRoutes.length > 0
                ? preloadedRoutes.reduce((sum, m) => sum + (m.duration || 0), 0) /
                  preloadedRoutes.length
                : 0;

        const nonPreloadedAverage =
            nonPreloadedRoutes.length > 0
                ? nonPreloadedRoutes.reduce((sum, m) => sum + (m.duration || 0), 0) /
                  nonPreloadedRoutes.length
                : 0;
        ...
        return {
            ...
            preloadingEffectiveness: {
                ...
                nonPreloadedAverage,
                improvement:
                    nonPreloadedAverage > 0
                        ? ((nonPreloadedAverage - preloadedAverage) / nonPreloadedAverage) * 100
                        : 0,
            },
        };
    }
    ...
    private calculateCacheHitRate(): number {
        const metricsWithCacheInfo = this.lazyLoadMetrics.filter((m) => m.cacheHit !== undefined);
    ...
    private calculateRetryRate(): number {
        const metricsWithRetryInfo = this.lazyLoadMetrics.filter((m) => m.retryCount !== undefined);
    ...
    checkPerformanceThresholds() {
        ...
        if (stats.lazyLoad.averageTime > 1000) {
            warnings.push(
                `Average lazy load time is high: ${stats.lazyLoad.averageTime.toFixed(2)}ms`
            );
        }

        if (stats.lazyLoad.retryRate > 10) {
            warnings.push(
                `High retry rate for lazy loading: ${stats.lazyLoad.retryRate.toFixed(2)}%`
            );
        }
        ...
        if (stats.routes.averageTime > 2000) {
            warnings.push(
                `Average route navigation time is high: ${stats.routes.averageTime.toFixed(2)}ms`
            );
        }

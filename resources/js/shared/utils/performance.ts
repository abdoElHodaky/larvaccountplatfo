...
export class PerformanceMonitor {
    ...
    public getReport(): PerformanceReport {
        ...
        const errorRate = timingMetrics.length > 0 ? errorMetrics.length / timingMetrics.length : 0;

        const currentMemory =
            memoryMetrics.length > 0 ? memoryMetrics[memoryMetrics.length - 1].value : 0;
    ...
    private checkThresholds(metric: PerformanceMetric): void {
        ...
        if (metric.name === 'memory_used' && metric.value > this.thresholds.memoryUsage) {
            console.warn(
                `High memory usage detected: ${(metric.value / 1024 / 1024).toFixed(0)}MB`
            );
        }
...
export function usePerformanceMonitor() {
    ...
    const recordTiming = React.useCallback(
        (name: string, duration: number, tags?: Record<string, string>) => {
            monitor.recordTiming(name, duration, tags);
        },
        [monitor]
    );

    const recordCounter = React.useCallback(
...
export const FinancialPerformanceUtils = {
    ...
    formatPercentage: memoize(
        (value: number, decimals: number = 2, locale: string = 'en-US'): string => {
            return new Intl.NumberFormat(locale, {
                style: 'percent',

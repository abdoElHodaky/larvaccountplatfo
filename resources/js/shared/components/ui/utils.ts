...
export function getFocusClasses(colorScheme: ColorScheme = 'primary') {
    const colors = getColorClasses(colorScheme);
    return cn('focus:outline-none', 'focus:ring-2', 'focus:ring-offset-2', colors.ring);
}
...
export function getValidationClasses(error?: string, success?: boolean) {
    ...
    return cn('border-gray-300', 'focus:border-primary-500', 'focus:ring-primary-500');
}
...
export function getAccountingColorClasses(type: 'debit' | 'credit' | 'balance', amount: number) {
    ...
    switch (type) {
        case 'debit':
            return amount > 0
                ? 'text-asset-600 dark:text-asset-400'
                : 'text-liability-600 dark:text-liability-400';
        case 'credit':
            return amount > 0
                ? 'text-liability-600 dark:text-liability-400'
                : 'text-asset-600 dark:text-asset-400';

...
export const AnimatedButton: React.FC<AnimatedButtonProps> = ({
  ...
  const buttonVariants = {
    initial: { scale: 1 },
    hover: enableHoverEffects && shouldAnimate && animationLevel === 'full' ? {
      scale: 1.02,
      y: -1,
      transition: { duration: 0.2, ease: 'easeOut' }
    } : { scale: 1 },
    tap: enableTapEffects && shouldAnimate ? {
      scale: 0.98,
 ...
 * Icon button variant
 */
export interface AnimatedIconButtonProps extends Omit<AnimatedButtonProps, 'children' | 'icon' | 'rightIcon'> {
  icon: React.ReactNode;
  'aria-label': string;
  tooltip?: string;
}

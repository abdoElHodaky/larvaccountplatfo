            ...
            >
                <div className='flex'>
                    <div className='flex-shrink-0'>
                        {icon || <div className={colorStyles.text}>{getDefaultIcon()}</div>}
                    </div>
                    <div className='ml-3 flex-1'>
                        {title && (
                            <h3 className={cn('text-sm font-medium', colorStyles.text)}>{title}</h3>
                        )}
...
export const Loading = forwardRef<HTMLDivElement, LoadingProps>(
    ({ size = 'md', text, overlay = false, className, ...props }, ref) => {
        const sizeStyles = getSizeClasses(size);
        ...
        const spinner = (
            <div className='flex flex-col items-center justify-center'>
                ...
                />
                {text && (
                    <p className={cn('text-gray-600 dark:text-gray-400 mt-2', sizeStyles.text)}>
                        {text}
                    </p>
                )}
            </div>
...
export const Skeleton = forwardRef<HTMLDivElement, SkeletonProps>(
    (
        ...
        const baseClasses = cn('bg-gray-200 dark:bg-gray-700 rounded', active && 'animate-pulse');
                    ...
                    {avatar && (
                        <div
                            className={cn(
                                ...
                                avatarConfig.size === 'sm'
                                    ? 'h-8 w-8'
                                    : avatarConfig.size === 'lg'
                                      ? 'h-16 w-16'
                                      : 'h-12 w-12'
                            )}
                        />
...
export const Empty = forwardRef<HTMLDivElement, EmptyProps>(
    ({ icon, title = 'No data', description, action, className, ...props }, ref) => {
        return (
            <div
                ref={ref}
                className={cn(
...
export { ToastProvider, useToast, Alert, Progress, Loading, Skeleton, Empty };

// Export types
export type { LoadingProps, EmptyProps };

...
export const SkeletonLoader: React.FC<SkeletonLoaderProps> = ({
  ...
  `.trim();
    ...
    const pulseAnimation =
        shouldAnimate && enableAnimation && animationLevel !== 'none'
            ? {
                  animate: {
                      opacity: [0.4, 0.8, 0.4],
...
export const CardSkeleton: React.FC<CardSkeletonProps> = ({
    ...
    <Fragment>
        ...
        >
            {showHeader && (
                <Fragment>
                    <div className='flex items-center space-x-3'>
                        {showAvatar && (
                            <Fragment>
                                <SkeletonLoader width={40} height={40} rounded={true} {...props} />
                            </Fragment>
                        )}
                        <div className='flex-1 space-y-2'>
                            <SkeletonLoader width='60%' height='1.25rem' {...props} />
                            <SkeletonLoader width='40%' height='0.875rem' {...props} />
                        </div>
                    </div>
                </Fragment>
            )}
            ...
            <Fragment>
                <TextSkeleton lines={contentLines} height='1rem' {...props} />
            </Fragment>
        </FragmentAnimationWrapper>
    </Fragment>
);
                    ...
                    >
                        {Array.from({ length: columns }, (_, index) => (
                            <Fragment key={`header-${index}`}>
                                <SkeletonLoader width='80%' height='1.25rem' {...props} />
                            </Fragment>
                        ))}
                    </div>
...
export const ChartSkeleton: React.FC<ChartSkeletonProps> = ({
        ...
        return (
            <Fragment>
                <div className={`flex items-center justify-center ${className}`} style={{ height }}>
                    <SkeletonLoader width={150} height={150} rounded={true} {...props} />
                </div>
            </Fragment>
        );
    }
    ...
    return (
        <Fragment>
            <div className={`space-y-2 ${className}`}>
                {/* Chart title */}
                <SkeletonLoader width='40%' height='1.5rem' {...props} />
                        ...
                        <Fragment key={index}>
                            <div className='flex-1 flex flex-col items-center space-y-2'>
                                <SkeletonLoader
                                    ...
                                    {...props}
                                />
                                <SkeletonLoader width='80%' height='0.75rem' {...props} />
                            </div>
                        </Fragment>
                    ))}
                </div>
    ...
    return (
        <Fragment>
            <div className={`flex items-center space-x-3 ${className}`}>
                <SkeletonLoader width={avatarSize} height={avatarSize} rounded={true} {...props} />
                {showName && (
                    <Fragment>
                        <div className='space-y-1'>
                            <SkeletonLoader width='120px' height='1rem' {...props} />
                            <SkeletonLoader width='80px' height='0.75rem' {...props} />
                        </div>
                    </Fragment>
                )}
            </div>

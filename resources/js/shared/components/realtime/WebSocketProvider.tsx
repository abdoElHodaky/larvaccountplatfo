...
export const WebSocketProvider: React.FC<WebSocketProviderProps> = ({ 
  ...
  const pendingMessages = useRef<Map<string, { resolve: (value: boolean) => void; reject: (reason: any) => void }>>(new Map());
    ...
    try {
      ...
      ws.onclose = (event) => {
        ...
        if (connectionStartTime.current) {
          stats.current.connectionUptime += Date.now() - connectionStartTime.current.getTime();
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'pong' && data.timestamp) {
            ...
            stats.current.averageLatency = latencyMeasurements.current.reduce((a, b) => a + b, 0) / latencyMeasurements.current.length;
            return;
          }
  ...
  }, [url, maxReconnectAttempts, reconnectInterval, enableCompression, processMessageQueue, startHeartbeat]);
  ...
  const sendMessage = useCallback(async (message: any, priority: 'high' | 'normal' | 'low' = 'normal'): Promise<boolean> => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (socket && socket.readyState === WebSocket.OPEN) {
        ...
        if (messageQueue.current.length >= messageQueueSize) {
          ...
          const lowPriorityIndex = messageQueue.current.findIndex(m => m.priority === 'low');

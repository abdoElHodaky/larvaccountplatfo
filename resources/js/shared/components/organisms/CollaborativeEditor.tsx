...
export const CollaborativeEditor: React.FC<CollaborativeEditorProps> = memo(
    ({
        ...
        useEffect(() => {
            ...
            unsubscribers.push(
                subscribe('document.user-left', (message) => {
                    setCollaborators((prev) => prev.filter((u) => u.id !== message.payload.userId));
                })
            );
        ...
        const applyOperation = useMemoizedCallback(
        ...
        const sendOperation = useMemoizedCallback(
        ...
        const handleContentChange = useMemoizedCallback(
            (newContent: string) => {
                if (readOnly) return;
        ...
        const cursorOverlays = useMemo(() => {
            return collaborators
                ...
                .map((user) => (
                    ...
                    >
                        {/* Cursor line */}
                        <Box w='2px' h='20px' bg={user.color} position='relative'>
                            {/* User label */}
                            <Box
                                position='absolute'
                                top='-25px'
...
function calculateDiff(
    oldContent: string,
    newContent: string
): Array<Omit<EditOperation, 'id' | 'userId' | 'timestamp'>> {
    ...
    if (newContent.length > oldContent.length) {
        ...
        const insertedContent = newContent.slice(
            insertPosition,
            insertPosition + (newContent.length - oldContent.length)
        );

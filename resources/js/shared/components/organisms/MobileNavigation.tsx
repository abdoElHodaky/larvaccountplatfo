...
import { Box, HStack, VStack, Text, Badge, useColorModeValue, Center } from '@chakra-ui/react';
...
export const MobileNavigation: React.FC<MobileNavigationProps> = memo(
    ({ items, onItemClick, activeItemId, className, variant = 'default' }) => {
...
const NavigationItem: React.FC<NavigationItemProps> = memo(
    ({ item, isActive, activeColor, inactiveColor, activeBg, onClick }) => {
 ...
 */
export const FinancialMobileNavigation: React.FC<Omit<MobileNavigationProps, 'items' | 'variant'>> =
    memo((props) => {
        ...
        return <MobileNavigation {...props} items={financialItems} variant='financial' />;
    });

...
export const TouchOptimizedTable: React.FC<TouchOptimizedTableProps> = memo(({
  ...
  // Search handler
  const handleSearch = useMemoizedCallback((value: string) => {
    setSearchTerm(value);
    if (onFilter) {
  ...
  // Sort handler
  const handleSort = useMemoizedCallback((column: string) => {
  ...
  const mobileFilters = useMemo(() => {
    ...
    return (
      <VStack spacing={3} p={4} bg={bgColor} borderBottom="1px solid" borderBottomColor={borderColor}>
        {/* Search */}
        <Input
          placeholder="Search..."
          value={searchTerm}
  ...
  const renderMobileRow = useMemoizedCallback((item: any, index: number) => {

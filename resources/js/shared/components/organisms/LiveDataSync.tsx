...
export const LiveDataSync: React.FC<LiveDataSyncProps> = memo(
    ({
        ...
        const handleConflict = useMemoizedCallback(
            (localChange: DataChange, remoteChange: DataChange) => {
        ...
        const resolveConflict = useMemoizedCallback(
            (localChange: DataChange, remoteChange: DataChange, resolution: ConflictResolution) => {
                ...
                setConflicts((prev) => prev.filter((c) => c.local.id !== localChange.id));
        ...
        const _queueChange = useMemoizedCallback(
            (change: Omit<DataChange, 'id' | 'timestamp'>) => {
        ...
        const syncChange = useMemoizedCallback(
            async (change: DataChange) => {
                try {
                    send('data.sync', {
                        change,
                        tenantId,

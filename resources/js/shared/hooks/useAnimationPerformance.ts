...
const detectDeviceCapabilities = (): DeviceCapabilities => {
  ...
  const memoryLevel: 'low' | 'medium' | 'high' = 
    memory <= 2 ? 'low' : 
    memory <= 4 ? 'medium' : 'high';

  // Detect connection speed
  const connection = (navigator as any).connection || (navigator as any).mozConnection || (navigator as any).webkitConnection;
  const connectionSpeed: 'slow' | 'medium' | 'fast' = 
    !connection ? 'fast' :
    connection.effectiveType === 'slow-2g' || connection.effectiveType === '2g' ? 'slow' :
...
export const useAnimationPerformance = (): AnimationPerformanceConfig => {
  ...
  const config = useMemo((): AnimationPerformanceConfig => {
    ...
    const duration = 
      animationLevel === 'none' ? 0.01 :
      animationLevel === 'minimal' ? animationConfig.duration.fast :
      animationLevel === 'reduced' ? animationConfig.duration.normal :
    ...
    const maxConcurrentAnimations = 
      animationLevel === 'minimal' ? 2 :
      animationLevel === 'reduced' ? 4 :
      8;

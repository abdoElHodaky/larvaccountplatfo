...
export interface UseInertiaFormReturn<T> {
  ...
  submit: (method: 'get' | 'post' | 'put' | 'patch' | 'delete', url: string, options?: InertiaFormOptions) => void;
...
export function useInertiaForm<T extends Record<string, any>>(
  ...
  const get = useCallback((url: string, options?: InertiaFormOptions) => {
    submit('get', url, options);
  }, [submit]);

  const post = useCallback((url: string, options?: InertiaFormOptions) => {
    submit('post', url, options);
  }, [submit]);

  const put = useCallback((url: string, options?: InertiaFormOptions) => {
    submit('put', url, options);
  }, [submit]);

  const patch = useCallback((url: string, options?: InertiaFormOptions) => {
    submit('patch', url, options);
  }, [submit]);

  const deleteMethod = useCallback((url: string, options?: InertiaFormOptions) => {

...
export function useCollaborativeDocument<T>(
  ...
  useEffect(() => {
    ...
    const unsubscribeJoin = on('collaboration:user_joined', (user: CollaborationUser) => {
      setCollaborators(prev => {
        ...
        if (existing) {
          return prev.map(u => u.id === user.id ? { ...u, ...user, lastSeen: new Date() } : u);
        }
    ...
    const unsubscribeChange = on('collaboration:change', (change: CollaborationChange) => {
      if (change.userId !== 'current-user-id') { // Don't apply own changes
        applyRemoteChange(change);
      }
    });

    // Cursor movements
    const unsubscribeCursor = on('collaboration:cursor', (data: {
      userId: string;
    ...
    // Conflicts
    const unsubscribeConflict = on('collaboration:conflict', (conflict: CollaborationConflict) => {
  ...
  const lockDocument = useCallback((lock: boolean) => {
...
export function useCollaborationSessions(organizationId?: number) {
  ...
  useEffect(() => {
    ...
    const unsubscribeSessions = on('collaboration:sessions_updated', (data: {
    ...
    const unsubscribeSessionCreated = on('collaboration:session_created', (session: CollaborationSession) => {
  ...
  const createSession = useCallback((documentId: string, documentType: string) => {

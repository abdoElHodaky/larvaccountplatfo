...
export function useChartData<T = any>(
  ...
  const processData = useMemoizedCallback((rawData: T[]): T[] => {
  ...
  const fetchData = useMemoizedCallback(async (useCache = true): Promise<void> => {
    try {
  ...
  const updateData = useMemoizedCallback((newData: T[]): void => {

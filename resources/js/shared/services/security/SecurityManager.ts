...
class SecurityManager {
    ...
    private detectSuspiciousPatterns(): void {
        ...
        accessByUser.forEach((count, userId) => {
            if (count > 100) {
                // More than 100 data access events in 5 minutes
                this.recordSecurityEvent({
                    type: 'suspicious_activity',
                    severity: 'medium',
    ...
    private updateSecurityMetrics(): void {
        ...
        this.securityMetrics = {
            ...
            policyViolations: this.events.filter((event) => event.type === 'policy_violation')
                .length,
    ...
    private terminateSession(sessionId: string, reason: string): void {
        ...
        document.dispatchEvent(
            new CustomEvent('auth:session_terminated', {

...
export class WebSocketManager {
  ...
  private handleDisconnection(): void {
    ...
    if (this.status.reconnectAttempts < this.options.maxReconnectAttempts) {
      ...
      this.log(`Reconnecting in ${this.options.reconnectInterval}ms (attempt ${this.status.reconnectAttempts})`);
...
export function useWebSocket(options?: WebSocketOptions) {
  ...
  const subscribe = React.useCallback((type: string, handler: WebSocketEventHandler) => {
    if (wsManager) {
...
export function useFinancialWebSocket(tenantId?: string) {
  ...
  const subscribeToTransactions = React.useCallback((handler: WebSocketEventHandler) => {
    return subscribe('transaction.created', handler);
  }, [subscribe]);

  const subscribeToAccountUpdates = React.useCallback((handler: WebSocketEventHandler) => {
    return subscribe('account.updated', handler);
  }, [subscribe]);

  const subscribeToReportUpdates = React.useCallback((handler: WebSocketEventHandler) => {
    return subscribe('report.generated', handler);
  }, [subscribe]);

  const subscribeToNotifications = React.useCallback((handler: WebSocketEventHandler) => {
    return subscribe('notification', handler);
  }, [subscribe]);

  const sendTransactionUpdate = React.useCallback((transactionId: string, data: any) => {
    send('transaction.update', { transactionId, data });
  }, [send]);

  const sendReportRequest = React.useCallback((reportType: string, params: any) => {
